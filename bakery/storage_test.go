...
func (*StorageSuite) TestMemStorage(c *gc.C) {
	...
	key, err := store.Get(context.Background(), []byte("x"))
	c.Assert(err, gc.Equals, bakery.ErrNotFound)
	c.Assert(key, gc.IsNil)

	key, err = store.Get(context.Background(), []byte("0"))
	c.Assert(err, gc.Equals, bakery.ErrNotFound)
	c.Assert(key, gc.IsNil)
	...
	key2, err := store.Get(context.Background(), id)
	c.Assert(err, gc.IsNil)
	c.Assert(key2, jc.DeepEquals, key)

	_, err = store.Get(context.Background(), []byte("1"))
	c.Assert(err, gc.Equals, bakery.ErrNotFound)
}
